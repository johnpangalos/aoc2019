...
func runIntcode(vals []int) {
	...
	for {
		...
		if c.op == display {
			...
			case foundWall:
				aMap[nCoord] = wall
				direction = nextDirectionClockwise(currCoord, aMap, direction)
			case moveDone:
				aMap[nCoord] = empty
				direction = nextDirectionCouterClockwise(currCoord, aMap, direction)
				currCoord = nCoord
			case foundTank:
				aMap[nCoord] = tank
				break
...
func nextDirectionClockwise(c coord, a areaMap, direction int) int {
	...
	// nextC := nextCoord(nextDirection, c)
	// if _, ok := a[nextC]; ok {
	// nextDirection = nextDirectionClockwise(nextC, a, nextDirection)
	// }

	return nextDirection
}

func nextDirectionCouterClockwise(c coord, a areaMap, direction int) int {
	var nextDirection int
	switch direction {
	case north:
		nextDirection = west
...
func (a areaMap) toString() string {
	...
	fmt.Println(minY, minX, maxY, maxX)
	for j := minY; j > maxY; j++ {
		for i := minX; i > maxX; i++ {
			v := a[coord{x: i, y: j}]
			switch v {
			...
			case empty:
				s = strings.Join([]string{s, "2"}, "")
			case tank:
				s = strings.Join([]string{s, "3"}, "")
			case start:
				s = strings.Join([]string{s, "4"}, "")
			}
		}
