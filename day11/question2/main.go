...
func main() {
	...
	minX, minY, maxX, maxY := 999999999, 999999999, 0, 0
	for k := range cMap {
		if k.x < minX {
			minX = k.x
		}
	...
	xLen := int(math.Abs(float64(minX)) + math.Abs(float64(maxX)))
	yLen := int(math.Abs(float64(minY)) + math.Abs(float64(maxY)))
	for j := yLen; j > -1; j-- {
		for i := 0; i < xLen+1; i++ {
			val, _ := cMap[coord{x: i + minX, y: j + minY}]
			if val == 0 {
				fmt.Printf("░")
			} else {
				fmt.Printf("█")
